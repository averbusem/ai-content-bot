...
НЕ ДОДУМЫВАЙ конкретику. Твоя задача - взять ЭТИ факты и подать их интересно."""
    ...
    async def edit_post(
        ...
        1️⃣ ЗАПРОС ПОЛЬЗОВАТЕЛЯ = ИСТИНА
        Вся информация из запроса пользователя - это РЕАЛЬНЫЕ факты, которые нужно:
        ✅ Добавить в пост
        ✅ Использовать для замены похожих неточных деталей

        2️⃣ НАХОДИ И ЗАМЕНЯЙ ПРОТИВОРЕЧИЯ
        Если в исходном посте есть информация, которая противоречит запросу:
        - Удали или замени её на информацию из запроса
        - Не оставляй конфликтующие детали

        3️⃣ СОХРАНЯЙ СТИЛЬ И СТРУКТУРУ
        - Тон и эмоциональность оригинала
        - Общую структуру (вступление, основная часть, завершение)
        - Эмодзи и форматирование (если не просят изменить)

        📋 ПРИМЕРЫ РЕДАКТИРОВАНИЯ:

        Пример 1:
        Исходный пост: "За два дня участники создали 5 крутых проектов..."
        Запрос: "Хакатон длился 3 дня и было 7 команд"
        Результат: "За три дня семь команд создали крутые проекты..."
        (убрали "два дня" и "5 проектов", добавили реальные "3 дня" и "7 команд")

        Пример 2:
        Исходный пост: "Собрали средства на корм для приюта"
        Запрос: "Добавь что собрали 45 000 рублей"
        Результат: "Собрали 45 000 рублей на корм для приюта"
        (добавили конкретную сумму)

        Пример 3:
        Исходный пост: "Волонтёры помогли животным в приюте"
        Запрос: "Это были Мария, Алексей и Ирина"
        Результат: "Волонтёры Мария, Алексей и Ирина помогли животным в приюте"
        (добавили имена)

        Пример 4:
        Исходный пост: "Скоро пройдёт акция по раздаче корма"
        Запрос: "Акция 20 декабря в 14:00 на площади Ленина"
        Результат: "20 декабря в 14:00 на площади Ленина пройдёт акция по раздаче корма"
        (добавили точные дату, время, место)

        Пример 5:
        Исходный пост: "Участники работали над проектами для добрых дел. Создали приложения и сервисы"
        Запрос: "Они разработали чат-бот для записи волонтёров и сайт сбора пожертвований"
        Результат: "Участники работали над проектами для добрых дел. Разработали чат-бот для записи волонтёров и сайт сбора пожертвований"
        (заменили общие "приложения и сервисы" на конкретные проекты)

        ⚙️ ПРАВИЛА:

        ✅ ОБЯЗАТЕЛЬНО:
        - Использовать ВСЮ информацию из запроса пользователя
        - Органично вплести её в текст
        - Убрать противоречащие детали из оригинала
        - Сохранить читабельность и естественность

        ✅ СТИЛИСТИЧЕСКИЕ ИЗМЕНЕНИЯ (если просят):
        "Сделай короче" → сократить текст, оставить суть
        "Добавь эмоций" → добавить эмодзи, восклицания, яркие слова
        "Официальнее" → убрать эмодзи, использовать формальный язык
        "Убери эмодзи" → удалить все эмодзи
        "Проще" → короткие предложения, простые слова

        ❌ НЕ ДЕЛАЙ:
        - Не игнорируй информацию из запроса
        - Не оставляй старые детали, если они конфликтуют с новыми
        - Не добавляй информацию, которой нет ни в посте, ни в запросе"""
        ...
        ИНСТРУКЦИЯ:
        1. Добавь всю информацию из запроса в пост
        2. Если в исходном посте есть похожая, но другая информация - замени её
        3. Сохрани стиль, структуру и тон оригинала
        4. Сделай текст естественным и читабельным
    ...
    async def edit_text(
        ...
        Args:
            ...
            edit_focus: Фокус редактирования (грамматика, орфография, логика, стиль)

        Returns:
            Словарь с отредактированным текстом и пояснениями
        """
        system_prompt = """Ты - профессиональный редактор и корректор текстов.
        Твоя задача - найти и исправить ошибки, дать рекомендации по улучшению."""

        prompt = f"""Проанализируй следующий текст и исправь его.
        Фокус редактирования: {edit_focus}
        ...
        НАЙДЕННЫЕ ОШИБКИ:
        1. [описание ошибки и как исправлено]
        2. [описание ошибки и как исправлено]
        ...
        ...
        for line in response.split('\n'):
            ...
            if "ИСПРАВЛЕННЫЙ ТЕКСТ:" in line:
                ...
                continue
            elif "НАЙДЕННЫЕ ОШИБКИ:" in line:
                current_section = "errors"
                continue
            elif "РЕКОМЕНДАЦИИ ПО УЛУЧШЕНИЮ:" in line:
