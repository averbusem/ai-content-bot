...
async def frequency_input_handler(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        return await message.answer(
            ...
            "<i>Примеры:</i>\n"
            "• \"Больше историй благополучателей\"\n"
            "• \"Меньше просьб о донатах\"\n"
            "• \"Фокус на образовательный контент\"\n\n"
...
async def preferences_input_handler(message: types.Message, state: FSMContext):
    ...
    preferences = None if preferences_text.lower() in ["нет", "не нужно", "без предпочтений"] else preferences_text
